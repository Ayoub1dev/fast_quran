...
function onCenterSize(v) { CFG.centerSize = +v; $('centerSizeVal').textContent = v; applyDisplaySettings(); saveSettings(); }
function onSideOpacity(v) { CFG.sideOpacity = +v; $('sideOpacityVal').textContent = v + '%'; applyDisplaySettings(); saveSettings(); }
function onSideSizeDiff(v) { CFG.sideSizeDiff = +v; $('sideSizeDiffVal').textContent = v + 'px'; applyDisplaySettings(); saveSettings(); }
...
//  RENDER WORD RAIL
// ═══════════════════════════════════════════════════
// Slot layout (RTL reading order):
//   ws0=idx-4  ws1=idx-3  ws2=idx-2  ws3=idx-1
//   ws4=idx (CENTER)
//   ws5=idx+1  ws6=idx+2  ws7=idx+3  ws8=idx+4
//
// Because Arabic reads right-to-left:
//   - words already read (idx-N) appear on the RIGHT
//   - upcoming words (idx+N) appear on the LEFT
// This matches the natural reading direction perfectly.

const SLOT_CLASSES = ['side4','side3','side2','side1','center','side1','side2','side3','side4'];
...
function renderRail() {
  for (let s = 0; s < 9; s++) {
    ...
    const word = words[wi];
    el.className = 'ws ' + SLOT_CLASSES[s];
    // Render with tajweed coloring for center word; plain text for sides
    if (s === 4) {
      el.innerHTML = buildTajweedHTML(word.a);
      el.classList.remove('pop');
      void el.offsetWidth;
      el.classList.add('pop');
    } else {
      // Side words: plain text (no color distracts from center)
      el.textContent = word.a;
    }
  }
