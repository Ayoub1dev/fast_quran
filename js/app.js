...
function buildRailDOM() {
  // Rebuild the slot elements to match current railSlots
  const rail = $('wordRail');
  rail.innerHTML = '';

  const { past, next } = railSlots;
  const total = past + 1 + next; // past slots + center + next slots

  // In RTL flex, the first child is on the RIGHT.
  // Past words (already read) go on the RIGHT → added first.
  // Center goes in the middle.
  // Next words (upcoming) go on the LEFT → added last.

  for (let p = past; p >= 1; p--) {
...
function renderRail() {
  ...
  // ── Center word ──
  const center = $('ws-center');
  if (center && words[idx]) {
    center.innerHTML = buildTajweedHTML(words[idx].a);
    center.className = 'ws center';
